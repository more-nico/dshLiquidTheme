window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function findSidebarInner() {
      const col = document.querySelector("#root > [data-slot] > div > div:first-child");
      if (!col) return null;
      for (const node of col.querySelectorAll(":scope > .dsh-lg-effect, :scope > .dsh-lg-shine")) node.remove();
      const slot = col.querySelector('[data-slot="sidebar"]');
      return slot ? slot.firstElementChild : null;
    }

    function decorateChrome() {
      mountLayers(findSidebarInner(), "sidebar");
