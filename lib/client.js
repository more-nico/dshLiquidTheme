window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function syncTracesClearance(traces) {
      ...
      const need = Math.round(header.getBoundingClientRect().bottom + 12 - traces.getBoundingClientRect().top);
      if (need > 0) traces.style.paddingTop = need + "px";
      else traces.style.removeProperty("padding-top");
    }
