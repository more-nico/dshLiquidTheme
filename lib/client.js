window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function readPrefs() {
      try {
        ...
        return {
          ...
          blur: clamp(parsed.blur, 0, 40, 6),
          refract: clamp(parsed.refract, 0, 100, 30),
          refractOn: parsed.refractOn !== false,
          scrim: clamp(parsed.scrim, 0, 100, 0),
          scrimBlur: clamp(parsed.scrimBlur, 0, 40, 0),
    ...
    function applyBlur(blur) {
      const body = document.body;
      if (!body) return;
      body.style.setProperty("--lg-blur", `${clamp(blur, 0, 40, 6)}px`);
    }
    ...
    function applyRefract(refract, on) {
      const body = document.body;
      if (!body) return;
      const n = on === false ? 0 : clamp(refract, 0, 100, 30) / 100;
    ...
    function syncRefraction() {
      ...
      const strength = prefs.refractOn === false ? 0 : clamp(prefs.refract, 0, 100, 30) / 100;
      const blur = clamp(prefs.blur, 0, 40, 6);
    ...
    function SettingsPanel(store, React) {
      return function LiquidGlassSettings() {
        ...
        const setBlur = (event) => {
          store.set({ blur: clamp(event.target.value, 0, 40, 6) });
        };
        const setRefract = (event) => {
          store.set({ refract: clamp(event.target.value, 0, 100, 30) });
        };
        const setRefractOn = () => {
        ...
        const setScrimBlur = (event) => {
          store.set({ scrimBlur: clamp(event.target.value, 0, 40, 0) });
        };
        const applyUrl = () => {
