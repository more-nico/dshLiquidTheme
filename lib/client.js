window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function readPrefs() {
      try {
        ...
        return {
          ...
          veil: clamp(parsed.veil, 0, 100, 0),
          scrim: clamp(parsed.scrim, 0, 100, 0),
    ...
    function applyVeil(veil) {
      const body = document.body;
      if (!body) return;
      body.style.setProperty("--lg-veil", String(clamp(veil, 0, 100, 0) / 100));
    }

    function applyScrim(scrim) {
      const body = document.body;
      if (!body) return;
      const n = clamp(scrim, 0, 100, 0) / 100;
      body.style.setProperty("--lg-scrim", String(n));
      if (n > 0) body.setAttribute("data-lg-scrim", "");
    ...
    const ZH = {
      ...
      veilTitle: "壁纸压纱",
      veilHint: "整张背景上盖一层浅纱，压低插画对比。0 为不压。",
      scrimTitle: "内容垫层",
      scrimHint: "对话中间一条渐变垫层，中间最实、两边完全透明。",
      apply: "应用",
      choose: "选择图片",
      reset: "恢复默认",
    ...
    const EN = {
      ...
      veilTitle: "Wallpaper veil",
      veilHint: "A wash over the whole wallpaper to calm busy images. 0 is off.",
      scrimTitle: "Reading scrim",
      scrimHint: "A gradient veil in the conversation column: solid in the middle, clear at the sides.",
      apply: "Apply",
      choose: "Choose image",
      reset: "Reset",
    ...
    function SettingsPanel(store, React) {
      return function LiquidGlassSettings() {
        ...
        const setVeil = (event) => {
          store.set({ veil: clamp(event.target.value, 0, 100, 0) });
        };
        const setScrim = (event) => {
          store.set({ scrim: clamp(event.target.value, 0, 100, 0) });
        };
        const applyUrl = () => {
        ...
        return React.createElement("div", { className: "dsh-lg-pref", "data-lg-settings": "" },
          ...
          React.createElement("div", { className: "dsh-lg-pref-card" },
            ...
            React.createElement("div", { className: "dsh-lg-pref-row" },
              React.createElement("div", { className: "dsh-lg-pref-copy" },
                React.createElement("div", { className: "dsh-lg-pref-title" }, t.veilTitle),
                React.createElement("div", { className: "dsh-lg-pref-hint" }, t.veilHint),
              ),
              React.createElement("div", { className: "dsh-lg-pref-meta" }, `${prefs.veil}%`),
            ),
            React.createElement("input", {
              className: "dsh-lg-pref-range",
              type: "range",
      ...
      ctx.effect(() => {
        ...
        const teardown = () => {
          ...
          if (document.body) {
            ...
            document.body.style.removeProperty("--lg-veil");
        ...
        const paint = (prefs) => {
          ...
          applyVeil(prefs.veil);
          applyScrim(prefs.scrim);
