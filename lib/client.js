window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const REFRACT_FILTERS = {
      ...
      todo: "dsh-lg-refract-todo",
      question: "dsh-lg-refract-question",
      planReview: "dsh-lg-refract-plan-review",
    ...
    function isColorWallpaper(value) {
      return /^color:/i.test(String(value || "").trim())
        || /^#([0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(String(value || "").trim());
    }
    ...
    function toColorPref(value) {
      ...
      if (short) {
        ...
        const b = short[1][2];
        const a = short[1][3];
        return `color:#${r}${r}${g}${g}${b}${b}${a ? `${a}${a}` : ""}`.toLowerCase();
      }
      if (/^#([0-9a-f]{6}|[0-9a-f]{8})$/i.test(raw)) return `color:${raw.toLowerCase()}`;
      return "";
    }
    ...
    function syncRefraction() {
      ...
      if (!body || !body.hasAttribute(ATTR) || reducedTransparency()) {
        for (const target of [
          document.querySelector("#root > [data-slot] > div > div:first-child"),
          findHeaderHost(),
          findComposerHost(),
          document.querySelector('[data-lg-host="traces"]'),
          document.querySelector('[data-slot="conversation.composer.dock"] > *'),
          findTodoPanel(),
          findQuestionCard(),
          findPlanReviewCard(),
          findJumpButton(),
      ...
      if (dock && !dock.matches('[data-testid="todo-panel"]')) {
      ...
      const todo = findTodoPanel();
      if (todo) {
        const box = todo.getBoundingClientRect();
        applyFx(
          todo,
          REFRACT_FILTERS.todo,
          box.width,
          box.height,
          readRadius(todo, 12),
          strength * 0.62,
      ...
      const question = findQuestionCard();
      if (question) {
        const box = question.getBoundingClientRect();
        applyFx(
          question,
          REFRACT_FILTERS.question,
          box.width,
          box.height,
          readRadius(question, 20),
          strength * 0.58,
          frost,
        );
      }

      const planReview = findPlanReviewCard();
      if (planReview) {
        const box = planReview.getBoundingClientRect();
        applyFx(
          planReview,
          REFRACT_FILTERS.planReview,
          box.width,
          box.height,
          readRadius(planReview, 20),
          strength * 0.58,
          frost,
        );
      }
    ...
    const INK_PRIMARY_CONTRAST = 4.5;
    const INK_SECONDARY_CONTRAST = 3;
    const INK_TERTIARY_CONTRAST = 3;
    const INK_GUARD_ENTER = 4.4;
    const INK_GUARD_EXIT = 4.65;
    const INK_COLOR_DELTA = 12;
    const INK_TEXT_ATTR = "data-lg-ink-text";
    const INK_GUARD_ATTR = "data-lg-ink-guard";
    ...
    let wallLoadGen = 0;
    let inkRaf = 0;
    let inkThrottleTimer = 0;
    let inkForceQueued = false;
    let inkOwnedNodes = new Set();
    let inkState = new WeakMap();
    ...
    function parseHexColor(hex) {
      ...
      if (!/^[0-9a-f]{6}([0-9a-f]{2})?$/i.test(raw)) {
        return { rgb: WALL_FALLBACK.slice(), alpha: 1 };
      }
      const n = parseInt(raw.slice(0, 6), 16);
      return {
        rgb: [(n >> 16) & 255, (n >> 8) & 255, n & 255],
        alpha: raw.length === 8 ? parseInt(raw.slice(6), 16) / 255 : 1,
      };
    }
    ...
    function ensureWallCanvas(prefs) {
      ...
      if (isColorWallpaper(prefs.wallpaper)) {
        const key = `color:${wallpaperHex(prefs.wallpaper)}`;
        if (key !== wallCanvasKey) {
          wallLoadGen += 1;
          clearInkTargets();
          applySafeInkFallback();
        }
    ...
    function sampleWallAt(vx, vy, prefs) {
      if (isColorWallpaper(prefs.wallpaper)) {
        const color = parseHexColor(wallpaperHex(prefs.wallpaper));
        if (color.alpha >= 1) return color.rgb;
        return blendRgb(isDarkTheme() ? [7, 16, 24] : [7, 20, 31], color.rgb, color.alpha);
      }
    ...
    function parseCssColor(value) {
      ...
      const alpha = m[4] === undefined
        ? 1
        : (m[4].endsWith("%") ? Number(m[4].slice(0, -1)) / 100 : Number(m[4]));
      return {
    ...
    function frostOver(rgb, kind, prefs, overlay) {
      if (reducedTransparency()) {
        if (overlay && overlay.alpha > 0) return blendRgb(rgb, overlay.rgb, overlay.alpha);
      ...
      const base = kind === "pad" || kind === "hero" ? rgb.slice() : toneRgb(rgb, contrast, bright);
      if (overlay && overlay.alpha > 0) return blendRgb(base, overlay.rgb, overlay.alpha);
      if (kind === "hero") return base;
      let out = base;
      if (kind === "pad") {
    ...
    function formatRgba(rgb, alpha) {
      return `rgba(${rgb.map((value) => Math.round(value)).join(", ")}, ${alpha})`;
    }
    ...
    function chooseInk(backgrounds, target) {
      const [hue] = rgb2hsl(...averageRgb(backgrounds));
      const candidates = [];
      for (const saturation of [0, 8, 16, 24, 32]) {
        for (let lightness = 0; lightness <= 100; lightness += 2) {
          const rgb = hsl2rgb(hue, saturation, lightness);
          const minContrast = Math.min(...backgrounds.map((bg) => contrastRatio(rgb, bg)));
          candidates.push({ rgb, saturation, minContrast });
        }
      }
      const passing = candidates.filter((candidate) => candidate.minContrast >= target);
      const pool = passing.length ? passing : candidates;
      return pool.reduce((best, candidate) => {
        if (!best) return candidate;
        if (passing.length && candidate.saturation !== best.saturation) {
          return candidate.saturation > best.saturation ? candidate : best;
        }
        if (candidate.minContrast !== best.minContrast) {
          return candidate.minContrast > best.minContrast ? candidate : best;
        }
        return candidate.saturation > best.saturation ? candidate : best;
      }, null);
    }

    function inkSignature(backgrounds) {
      return backgrounds
        .map((rgb) => rgb.map((value) => Math.round(value / 8)).join(","))
        .sort()
        .join("|");
    }

    function makeInkResult(backgrounds) {
      if (!backgrounds || !backgrounds.length) return null;
      const primary = chooseInk(backgrounds, INK_PRIMARY_CONTRAST);
      const secondary = chooseInk(backgrounds, INK_SECONDARY_CONTRAST);
      const tertiary = chooseInk(backgrounds, INK_TERTIARY_CONTRAST);
      const guardFill = relLum(primary.rgb) < 0.5 ? [255, 255, 255] : [0, 0, 0];
      return {
        primary: primary.rgb,
        secondary: secondary.rgb,
        tertiary: tertiary.rgb,
        primaryContrast: primary.minContrast,
        guardFill,
        needsGuard: primary.minContrast < INK_GUARD_ENTER,
        signature: inkSignature(backgrounds),
      };
    }

    function safeInkResult() {
      const bg = isDarkTheme() ? [28, 30, 40] : [247, 248, 251];
      const result = makeInkResult([bg]);
      result.needsGuard = false;
      return result;
    }
    ...
    function visibleBox(el) {
      ...
      const right = Math.min(window.innerWidth || 0, box.right);
      const bottom = Math.min(window.innerHeight || 0, box.bottom);
      if (box.width < 2 || box.height < 2 || right - left < 2 || bottom - top < 2) return null;
      return { left, top, width: right - left, height: bottom - top };
    }

    function sampleBoxRgb(entry, prefs) {
      const box = visibleBox(entry.el);
      if (!box) return null;
      const points = [];
      for (const y of [0.1, 0.3, 0.5, 0.7, 0.9]) {
        for (const x of [0.1, 0.3, 0.5, 0.7, 0.9]) points.push([x, y]);
      }
      const ownBg = parseCssColor(getComputedStyle(entry.el).backgroundColor);
      const surfaceBg = entry.surface && entry.surface !== entry.el
        ? parseCssColor(getComputedStyle(entry.surface).backgroundColor)
        : null;
      const overlay = ownBg && ownBg.alpha > 0 ? ownBg : surfaceBg;
      const backgrounds = [];
      for (const [px, py] of points) {
        const under = sampleWallAt(box.left + box.width * px, box.top + box.height * py, prefs);
        if (!under) continue;
        backgrounds.push(frostOver(under, entry.kind, prefs, overlay));
      }
      if (!backgrounds.length) return null;
      const result = makeInkResult(backgrounds);
      if (!result) return null;
      result.signature = `${Math.round(box.left / 8)},${Math.round(box.top / 8)},${Math.round(box.width / 8)},${Math.round(box.height / 8)}:${result.signature}`;
      return result;
    }

    function hasDirectText(el) {
      return [...el.childNodes].some((node) => node.nodeType === 3 && /\S/.test(node.nodeValue || ""));
    }

    function isProtectedInkNode(el) {
      const selector = "a, pre, code, kbd, samp, blockquote, .dsh-lg-pref-error, [data-variant], [role='status'], [role='alert'], [aria-live]";
      return el.matches(selector) || !!el.closest(selector);
    }

    function isMirroredComposerInput(el) {
      const card = el.matches("textarea") ? el.closest("[data-composer-card]") : null;
      return !!card && !!card.querySelector("[data-input-backdrop]");
    }

    function hasProtectedInkDescendant(el) {
      const selector = "a, pre, code, kbd, samp, blockquote, .dsh-lg-pref-error, [data-variant], [role='status'], [role='alert'], [aria-live]";
      return isProtectedInkNode(el) || !!el.querySelector(selector);
    }

    function isVisibleInkNode(el) {
      const box = el.getBoundingClientRect();
      return box.width >= 2 && box.height >= 2 && getComputedStyle(el).visibility !== "hidden"
        && getComputedStyle(el).display !== "none";
    }

    function canInkGuard(el) {
      if (el.matches("svg, textarea, input, [contenteditable='true'], button, [role='button'], [role='tab'], [role='option']")) return false;
      if (el.querySelector("svg")) return false;
      const text = (el.textContent || "").trim();
      if (!text || text.length > 160) return false;
      const style = getComputedStyle(el);
      if (["inline", "inline-block", "inline-flex"].includes(style.display)) return true;
      if (el.matches("h1, h2, h3, h4, h5, h6, [role='heading'], .dsh-lg-pref-heading, .dsh-lg-pref-title")) {
        return el.getBoundingClientRect().height <= 72;
      }
      return false;
    }

    function nearestInkSurface(el, fallback) {
      return el.closest('[data-lg-pad], [data-lg-host], [role="dialog"], [role="menu"], [role="listbox"]')
        || fallback || el;
    }

    function collectSemanticTargets(root, kind, fallback, out) {
      if (!root) return;
      const controls = "button, [role='button'], [role='tab'], [role='option'], textarea, input:not([type='range']):not([type='color']), [contenteditable='true']";
      const labels = "span, p, li, label, h1, h2, h3, h4, h5, h6, small, [role='heading'], .dsh-lg-pref-heading, .dsh-lg-pref-title, .dsh-lg-pref-hint, .dsh-lg-pref-lead, .dsh-lg-pref-meta";
      const add = (el) => {
        if (!el || isMirroredComposerInput(el) || !isVisibleInkNode(el) || isProtectedInkNode(el)) return;
        out.push({
          el,
          kind,
          surface: nearestInkSurface(el, fallback),
          allowGuard: canInkGuard(el),
        });
      };
      const controlNodes = root.matches(controls) ? [root] : [...root.querySelectorAll(controls)];
      for (const el of controlNodes) {
        if (!hasDirectText(el) && !el.querySelector("svg") && !el.matches("textarea, input, [contenteditable='true']")) continue;
        if (hasProtectedInkDescendant(el)) continue;
        add(el);
        for (const child of el.children) {
          if (child.matches("span, label") && (hasDirectText(child) || !child.children.length)) add(child);
          if (child.matches("svg")) add(child);
        }
      }
      for (const el of (root.matches(labels) ? [root] : [...root.querySelectorAll(labels)])) {
        if (!hasDirectText(el) && el.children.length) continue;
        if (hasProtectedInkDescendant(el)) continue;
        if ((el.textContent || "").trim()) add(el);
      }
    }

    function collectChatTextTargets(root, kind, out) {
      if (!root) return;
      const selectors = "p, li, h1, h2, h3, h4, h5, h6";
      const nodes = root.matches(selectors) ? [root] : [...root.querySelectorAll(selectors)];
      for (const el of nodes) {
        if (!isVisibleInkNode(el) || hasProtectedInkDescendant(el)) continue;
        if ((el.textContent || "").trim()) out.push({ el, kind, surface: root, allowGuard: canInkGuard(el) });
      }
      if (!nodes.length && !hasProtectedInkDescendant(root) && (root.textContent || "").trim()) {
        out.push({ el: root, kind, surface: root, allowGuard: false });
      }
    }
    ...
    function inkTargets() {
      const out = [];
      const sidebar = document.querySelector("#root > [data-slot] > div > div:first-child");
      if (sidebar) collectSemanticTargets(sidebar, "sidebar", sidebar, out);
      const hero = document.querySelector('[data-slot="conversation.hero.workspace"]');
      if (hero) collectSemanticTargets(hero, "hero", hero, out);
      ...
      if (document.body.hasAttribute("data-lg-home") && composerSlot
        && !document.querySelector('[data-lg-host="composer"]')) {
        collectSemanticTargets(composerSlot, "hero", composerSlot, out);
      }
      for (const el of document.querySelectorAll("[data-lg-host]")) {
        collectSemanticTargets(el, el.getAttribute("data-lg-host") || "host", el, out);
      }
      const dock = document.querySelector('[data-slot="conversation.composer.dock"] > *');
      if (dock) collectSemanticTargets(dock, "dock", dock, out);
      const dialog = document.querySelector("[role='dialog']");
      if (dialog) collectSemanticTargets(dialog, "dialog", dialog, out);
      for (const menu of document.querySelectorAll("[role='menu'], [role='listbox']")) {
        collectSemanticTargets(menu, "menu", menu, out);
      }
      for (const pad of document.querySelectorAll('[data-lg-pad="user"], [data-lg-pad="assistant"]')) {
        collectChatTextTargets(pad, "pad", out);
      }
      const settings = document.querySelector('[data-slot="sidebar.settings"], [data-slot="settings.trigger"]');
      if (settings) collectSemanticTargets(settings, "sidebar", sidebar || settings, out);
      return out;
    }

    function inkColorDistance(a, b) {
      return Math.abs(a[0] - b[0]) + Math.abs(a[1] - b[1]) + Math.abs(a[2] - b[2]);
    }

    function inkVisualKey(result) {
      return [result.primary, result.secondary, result.tertiary, result.needsGuard ? result.guardFill : null, result.needsGuard]
        .map((value) => Array.isArray(value) ? value.join(",") : String(value))
        .join("|");
    }

    function settleInkResult(entry, result) {
      const previous = inkState.get(entry.el);
      const previousResult = previous && previous.result;
      const guardThreshold = previousResult && previousResult.needsGuard
        ? INK_GUARD_EXIT
        : INK_GUARD_ENTER;
      const candidate = {
        ...result,
        needsGuard: !!entry.allowGuard && result.primaryContrast < guardThreshold,
      };
      if (!previousResult) return { result: candidate, pending: null };

      if (inkColorDistance(previousResult.primary, candidate.primary) <= INK_COLOR_DELTA
        && previousResult.needsGuard === candidate.needsGuard) {
        return {
          result: {
            ...previousResult,
            signature: candidate.signature,
            primaryContrast: candidate.primaryContrast,
          },
          pending: null,
        };
      }

      const key = inkVisualKey(candidate);
      const pending = previous.pending;
      if (!pending || pending.key !== key) {
        return { result: previousResult, pending: { key, result: candidate, count: 1 } };
      }
      return { result: candidate, pending: null };
    }

    function paintReadableInk(entry, result) {
      const previous = inkState.get(entry.el);
      const settled = settleInkResult(entry, result);
      const next = settled.result;
      const changed = !previous || inkVisualKey(previous.result) !== inkVisualKey(next)
        || !entry.el.hasAttribute(INK_TEXT_ATTR);
      inkState.set(entry.el, { result: next, pending: settled.pending });
      if (!changed) return;
      entry.el.setAttribute(INK_TEXT_ATTR, "");
      if (next.needsGuard) entry.el.setAttribute(INK_GUARD_ATTR, "");
      else entry.el.removeAttribute(INK_GUARD_ATTR);
      entry.el.style.setProperty("--lg-ink", formatRgb(next.primary));
      entry.el.style.setProperty("--lg-ink-soft", formatRgb(next.secondary));
      entry.el.style.setProperty("--lg-ink-faint", formatRgb(next.tertiary));
      entry.el.style.setProperty("--lg-ink-guard", formatRgba(next.guardFill, 0.18));
      inkOwnedNodes.add(entry.el);
    }

    function clearInkVars(el) {
      if (!el) return;
      el.removeAttribute(INK_TEXT_ATTR);
      el.removeAttribute(INK_GUARD_ATTR);
      ...
      el.style.removeProperty("--lg-ink-guard");
      inkOwnedNodes.delete(el);
      inkState.delete(el);
    }

    function clearInkTargets() {
      for (const node of inkOwnedNodes) clearInkVars(node);
      for (const node of document.querySelectorAll("[data-lg-ink-text], [data-lg-ink], [data-lg-ink-guard]")) {
        clearInkVars(node);
      }
    }
    ...
    function requestInkThrottled(force) {
      if (force) inkForceQueued = true;
      if (inkThrottleTimer) return;
      inkThrottleTimer = window.setTimeout(() => {
        inkThrottleTimer = 0;
        requestInk(false);
      }, 96);
    }
    ...
    function cancelInk() {
      ...
      if (inkThrottleTimer) {
        window.clearTimeout(inkThrottleTimer);
        inkThrottleTimer = 0;
      }
      inkForceQueued = false;
    }
    ...
    function clearInk() {
      if (document.body) document.body.removeAttribute("data-lg-ink-auto");
      clearInkTargets();
    }

    function applySafeInkFallback() {
      const result = safeInkResult();
      const seen = new Set();
      for (const entry of inkTargets()) {
        if (!entry.el || seen.has(entry.el)) continue;
        seen.add(entry.el);
        paintReadableInk(entry, result);
      }
    }
    ...
    function syncInk(force) {
      ...
      const imageAvailable = isColorWallpaper(prefs.wallpaper) || !!wallPixels;
      const seen = new Set();
      for (const entry of inkTargets()) {
        const el = entry.el;
        if (!el || el === body || seen.has(el)) continue;
        seen.add(el);
        const result = imageAvailable ? sampleBoxRgb(entry, prefs) : safeInkResult();
        if (result) paintReadableInk(entry, result);
        else clearInkVars(el);
      }
      for (const node of [...inkOwnedNodes]) {
    ...
    function stripChrome() {
      ...
      const dock = document.querySelector('[data-slot="conversation.composer.dock"] > *');
      clearFx(dock);
      clearFx(findTodoPanel());
      clearFx(findQuestionCard());
      clearFx(findPlanReviewCard());
      refractCache.clear();
    }
    ...
    function findTodoPanel() {
      return document.querySelector('[data-testid="todo-panel"]');
    }

    function findQuestionCard() {
      return document.querySelector('[data-question-key] > section');
    }

    function findPlanReviewCard() {
      return document.querySelector('[data-plan-review-key] > section');
    }
    ...
    const ZH = {
      ...
      inkAutoTitle: "自适应字色",
      inkAutoHint: "按 WCAG 对比度选可读字色，复杂背景只给文字加局部保护。",
      refractTitle: "强度",
      refractHint: "0 只留霜面。",
      padGroup: "对话垫层",
    ...
    const EN = {
      ...
      inkAutoHint: "Chooses WCAG-readable text colors and adds a local guard on complex backgrounds.",
